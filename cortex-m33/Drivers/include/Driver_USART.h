...
typedef struct {
        ...
        FT_UART0_Type          	*reg;              // Pointer to UART peripheral
				DRVUART_PORT						port;								// UART port number
				IRQn_Type               irq_num;           // UART IRQ number
        uint32_t            		clock;            	// UART clock
				uint32_t                rx_trig_lvl;        // RX FIFO Trigger level
				uint32_t                tx_trig_lvl;        // TX FIFO Trigger level
