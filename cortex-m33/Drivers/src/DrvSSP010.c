...
void fLib_I2S_Init(UINT32 base_address, UINT32 codec, SSP_MODE_T mode)
{
	UINT32 data;
...
void fLib_TXRX_Enable(UINT32 base_address, int tx, int rx)
{
	UINT32 cr2 = 0;
	
	if (tx || rx)
		cr2 = (SSP_SSPEN | SSP_TXDOE);
	...
	outw(base_address + SSP_CONTROL2, cr2);
}
...
UINT32 fLib_SSP_GetTxFIFOValidEntries(UINT32 base_addr)
{
	return (fLib_ReadSSPStatus(base_addr) & SSP_TFVE);
}
...
UINT32 fLib_SSP_busy(UINT32 base_addr)
{
	return (((fLib_ReadSSPStatus(base_addr) ) >> 2) & 0x1);
}

void fLib_SetSSP_SDL_Write(UINT32 base_address,UINT32 len,UINT32 w_data)
{
